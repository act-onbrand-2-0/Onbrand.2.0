...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check which users are online (they'll get push notification via DB trigger)
    // Only send emails to OFFLINE users
    ...
    if (resend && createdShares && createdShares.length > 0) {
      ...
      // Get online status for all recipients
      const { data: onlineStatuses } = await serviceSupabase
        .rpc('get_users_online_status', { p_user_ids: newUserIds });
      
      const onlineUserIds = new Set(
        (onlineStatuses || [])
          .filter((s: { user_id: string; is_online: boolean }) => s.is_online)
          .map((s: { user_id: string; is_online: boolean }) => s.user_id)
      );
      
      // Filter to only offline users for email
      const offlineUserIds = newUserIds.filter((id: string) => !onlineUserIds.has(id));
      
      console.log(`Sharing chat: ${newUserIds.length} recipients, ${onlineUserIds.size} online (push only), ${offlineUserIds.length} offline (will email)`);
      
      // Send emails only to offline users (don't block the response)
      if (offlineUserIds.length > 0) {
        Promise.all(
          offlineUserIds.map(async (recipientId: string) => {
            try {
              ...
              if (recipientDetails.email && recipientDetails.email !== 'Unknown') {
                ...
                console.log(`Email notification sent to ${recipientDetails.email} (user was offline)`);
              }
            } catch (emailError) {
